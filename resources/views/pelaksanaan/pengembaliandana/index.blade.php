            ...
            <div class="panel-body">
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <strong>Pengembalian Dana</strong> <small class="label label-success">{{$total}}</small>
                    </div>
                                        ...
                                        <li>
                                            <div class="row">
                                                ...
                                                <div class="tools col-sm-12 pull-left">
                                                    @if($periodeterbaru->tm_pengembaliandana == null && $periodeterbaru->tm_pengembaliandana == null)
                                                        <span class="text bg-red text-dark">&nbsp;Waktu Untuk Upload Data Belum di Buka</span>
                                                    @elseif($waktu < $periodeterbaru->tm_pengembaliandana )
                                                        <span class="text text-dark">Waktu Untuk Upload Data di Buka Pada Tanggal</span> <span class="text bg-blue text-dark">&nbsp;{{$periodeterbaru->tm_pengembaliandana}}</span> - <span class="text bg-red text-dark">&nbsp;{{$periodeterbaru->tm_pengembaliandana}}</span>
                                                    @else

                                                        @if($waktu >= $periodeterbaru->tm_pengembaliandana && $waktu <= $periodeterbaru->ta_pengembaliandana )
                                                            @if($detail->aktif == 1 && $detail->upload == null )
                                                                <a href="{{ route('pengembaliandana.edit',base64_encode(mt_rand(10,99).$detail->prosalid) )}}"  class="btn btn-app btn-sm" ><i class="ion ion-ios-cloud-upload-outline text-blue"></i> Upload </a>
                                                            @elseif ($detail->status > 0 && $detail->upload != null)
                                                                <a  href="{{ route('pengembaliandana.show',base64_encode(mt_rand(10,99).$detail->prosalid) )}}" class="btn btn-app btn-sm" id="baca"><i class="ion ion-ios-book-outline text-blue"></i> Baca </a>
                                                                <a href="{{ route('pengembaliandana.edit',base64_encode(mt_rand(10,99).$detail->prosalid) )}}" class="btn btn-app btn-xs" id="edit"><i class="ion ion-edit text-red"></i> Validasi </a>
                                                        ...
                                                        @elseif($waktu > $periodeterbaru->ta_pengembaliandana)
...
<script type="text/javascript">
 ...
 function hapusProposal(id) {
        if (confirm("Apakah yakin data dari kegiatan ini akan dihapus?")) {
            $.ajax({
                url  : "{{route('pengembaliandana.destroy','')}}/"+id,
                type : "POST",
                data : {'_method' : 'DELETE', '_token' : $('input[name = "_token"]').val()},
                success : function(data) {
                    window.location = "{{route('pengembaliandana.index')}}";
                },
                error : function() {
    ...
    function upload(id) {
        window.location = "{{route('pengembaliandana.edit', '')}}/"+btoa(id);
    }
