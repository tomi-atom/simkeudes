                ...
                <div class="panel-body">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <strong>Penggunaan Anggaran</strong> <small class="label label-success">{{$total}}</small>
                        </div>
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="tools col-sm-12 pull-left">
                                                         @if($detail->aktif == 1 && $detail->upload == null )
                                                                    <a href="{{ route('penggunaananggaran.edit',base64_encode(mt_rand(10,99).$detail->prosalid) )}}"  class="btn btn-app btn-sm" ><i class="ion ion-ios-cloud-upload-outline text-blue"></i> Upload </a>
                                                                @elseif ($detail->status > 0 && $detail->upload != null)
                                                                    <a  href="{{ route('penggunaananggaran.show',base64_encode(mt_rand(10,99).$detail->prosalid) )}}" class="btn btn-app btn-sm" id="baca"><i class="ion ion-ios-book-outline text-blue"></i> Baca </a>
                                                                ...
                                                                @endif
                                                        <!--
                                                        @if($periodeterbaru->tm_anggaran == null && $periodeterbaru->tm_anggaran == null)
                                                            <span class="text bg-red text-dark">&nbsp;Waktu Untuk Upload Data Belum di Buka</span>
                                                        @elseif($waktu < $periodeterbaru->tm_anggaran )
                                                            <span class="text text-dark">Waktu Untuk Upload Data di Buka Pada Tanggal</span> <span class="text bg-blue text-dark">&nbsp;{{$periodeterbaru->tm_anggaran}}</span> - <span class="text bg-red text-dark">&nbsp;{{$periodeterbaru->tm_anggaran}}</span>
                                                        @else

                                                            @if($waktu >= $periodeterbaru->tm_anggaran && $waktu <= $periodeterbaru->ta_anggaran )
                                                               

                                                            @elseif($waktu > $periodeterbaru->ta_anggaran)
    ...
    <script type="text/javascript">
        ...
        function hapusProposal(id) {
            if (confirm("Apakah yakin data dari kegiatan ini akan dihapus?")) {
                $.ajax({
                    url  : "{{route('penggunaananggaran.destroy','')}}/"+id,
                    type : "POST",
                    data : {'_method' : 'DELETE', '_token' : $('input[name = "_token"]').val()},
                    success : function(data) {
                        window.location = "{{route('penggunaananggaran.index')}}";
                    },
                    error : function() {
        ...
        function upload(id) {
            window.location = "{{route('penggunaananggaran.edit', '')}}/"+btoa(id);
        }
