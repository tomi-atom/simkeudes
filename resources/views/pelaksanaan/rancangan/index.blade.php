            ...
            <div class="panel-body">
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <strong>Rancangan Pelaksanaan</strong> <small class="label label-success">{{$total}}</small>
                    </div>
                                        ...
                                        <li>
                                            <div class="row">
                                                ...
                                                <div class="tools col-sm-12 pull-left">
                                                    @if($periodeterbaru->tm_rancangan == null && $periodeterbaru->tm_rancangan == null)
                                                        <span class="text bg-red text-dark">&nbsp;Waktu Untuk Upload Data Belum di Buka</span>
                                                    @elseif($waktu < $periodeterbaru->tm_rancangan )
                                                        <span class="text text-dark">Waktu Untuk Upload Data di Buka Pada Tanggal</span> <span class="text bg-blue text-dark">&nbsp;{{$periodeterbaru->tm_rancangan}}</span> - <span class="text bg-red text-dark">&nbsp;{{$periodeterbaru->tm_rancangan}}</span>
                                                    @else

                                                    @if($waktu >= $periodeterbaru->tm_rancangan && $waktu <= $periodeterbaru->ta_rancangan )
                                                        ...
                                                        @elseif($waktu > $periodeterbaru->ta_rancangan)
...
<script type="text/javascript">
 ...
 function hapusProposal(id) {
        if (confirm("Apakah yakin proposal dari kegiatan ini akan dihapus?")) {
            $.ajax({
                url  : "{{route('rancangan.destroy','')}}/"+id,
                type : "POST",
                data : {'_method' : 'DELETE', '_token' : $('input[name = "_token"]').val()},
                success : function(data) {
                    window.location = "{{route('rancangan.index')}}";
                },
                error : function() {
    ...
    function upload(id) {
        window.location = "{{route('rancangan.edit', '')}}/"+btoa(id);
    }
