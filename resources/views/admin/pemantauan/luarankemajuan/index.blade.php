            ...
            <div class="panel panel-primary">
                ...
                <div class="panel-body">
                    <div class="panel panel-default">
                        ...
                        <div class="panel-heading"><strong>Daftar Luaran Kemajuan  </strong><div class="pull-right"><strong></strong></div></div>
                        <div class="panel-body">
                            <div class="row">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="filter_thn" id="filter_thn" class="form-control" required>
                                                ...
                                                @foreach($periode as $listperiode)
                                                    @if($listperiode->tm_catatan == null && $listperiode->ta_catatan== null)
                                                    ...
                                                    @else
                                                        <option value="{{ $listperiode->id }}">{{ $listperiode->tahun }} sesi {{ $listperiode->sesi }} - @if($listperiode->jenis==1)<span>Penelitian</span> @else <span>Pengabdian </span> @endif  - <span class="text text-green">Mulai : {{ $listperiode->tm_catatan }} </span><span class="text text-green">- Akhir : {{ $listperiode->ta_catatan }} -</span>@if($listperiode->aktif == 1) Aktif @else Tidak Aktif @endif </option>
...
@section('script')
    <script type="text/javascript">
        ...
        function bacaProposalRinci(id) {
            window.location = "{{route('p_luarankemajuan.resume', '')}}/"+btoa(id);
        }
                    ...
                    $.ajax({
                        ...
                        success: function (result) {
                            ...
                            if (result.success) {
                                ...
                                let sesi = json.sesi;
                                $('.id').val(json.id);
                                $('.tm_catatan').val(json.tm_catatan);
                                $('.ta_catatan').val(json.ta_catatan);
